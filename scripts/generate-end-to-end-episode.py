...
def main():
    video_id = "overthinking-mind-fast-test"
    brief_text = (
        "POV: You quiet your overthinking mind FAST. You are trapped in an analytical loop of "
        "decision paralysis, trying to solve every life choice with infinite data. We trace this "
        "mental mess (Act 1), deep dive into the hidden cost of passive accumulation (Act 2), "
        "mirror your excuses back to you (Act 3), and offer subtractive preservation—the sweet "
    ...
    print("=========================================================")
    print("🎬 RESUME-ENABLED END-TO-END VIDEO GENERATION PIPELINE 🎬")
    print("=========================================================")
    ...
    if script is None:
        print(f"STAGE 1: GENERATING SCRIPT FOR brief: '{brief_text[:60]}...'")
        script_payload = {
    ...
    title = script.get("workingTitle", "Overthinking Mind Fast")
    print(f"Title: {title}")
    print(f"Archetype: {script.get('narrativeArchetype')}")
    
    # Normalize act IDs inside the script
        ...
        for block_idx, block in enumerate(act.get("narrationBlocks", [])):
            ...
            if os.path.exists(audio_abs_path) or os.path.exists(audio_abs_path_mp3):
                ...
                if audio_res.get("ok"):
                    ...
                    print(f"    ✗ Audio generation failed: {audio_res}")
                    sys.exit(1)
    ...
    print("\n=========================================================")
    print("STAGE 3: ASSEMBLING & EXPORTING ASSEMBLY.MP4 VIDEO ESSAY")
    print("=========================================================")
    export_payload = {
        "videoId": video_id,
        "workingTitle": title,
